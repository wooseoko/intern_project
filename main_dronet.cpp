...
void callback_collision_prob(std_msgs::Float32 temp);
int main(int argc, char** argv){
	...
	ros::Subscriber collision_probability = n.subscribe("collision_prob",1,callback_collision_prob);
...
//			        temp_goal.th = path_RRT[current_goal].th;
		    ...
		    speed_desired = 1.0 - collision_prob/2;
		    if(speed_desired <=0.0)
			cmd.drive.steering_angle_velocity = 0;
		    cmd.drive.speed = cmd.drive.speed * 0.7 + speed_desired * 0.3;
		    if(steer > 0.4)
		    {
			steer_desired = 0.8;
		    }
		    ...
		    cmd.drive.steering_angle = cmd.drive.steering_angle * 0.5 + steer_desired*0.5;
...
//			        float check_y = robot_pose.y - path_RRT[current_goal].y;
		    printf(" steer : %.3f      desired : %.3f     collision_prob : %.3f  \n",steer, steer_desired, collision_prob);

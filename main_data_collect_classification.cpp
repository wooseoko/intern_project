...
std::vector<cv::Mat> dImg1;
cv::Mat dImg;
std::vector<cv::Mat> dImg_total;
std::vector<cv::Mat> dImg_temp;
...
int main(int argc, char** argv){
	...
	int before_steer = 3; 
    // Initialize topics
    ros::Subscriber gazebo_pose_sub = n.subscribe("/gazebo/model_states", 5, callback_state);
    ros::Subscriber laser_sub = n.subscribe("/scan", 100, callback_laser);

    ros::Subscriber image_sub = n.subscribe("/camera/zed/rgb/image_rect_color", 5, callback_image);
    ...
    */
	...
	point x;
	point y;
    rrtTree *tree = new rrtTree(x, y, map, map_origin_x, map_origin_y, res, 30);
	map_margin = tree->map;
	delete tree;
	int temp = 0;
	int cnt_s = 0;
	int cnt_s_after = 0;
	int cnt_r = 0;
	int steer_folder[500];
	int steer_folder_total[2000];
	int index_temp = 0;
	int index_ = 0;
	int index_total = 0;
    ros::Rate control_rate(80);
    for(int iter = 0; iter < 2000; iter++) {
		bool collision = false;
			while(1){
				set_waypoints();
				if( ((waypoints[0].x - waypoints[1].x) * (waypoints[0].x - waypoints[1].x)) +( (waypoints[0].y - waypoints[1].y)*(waypoints[0].y - waypoints[1].y)) > 100 ) break;
			}
...
/*        bool valid = generate_path_RRT();
		...
		int inum = rand() % 10000;
        oss << "/home/wooseoko/catkin_ws/src/project2/color_data/" << inum;
		oss1 << "/home/wooseoko/catkin_ws/src/project2/color_data/";
        ...
        while(running) {
            ...
            case RUNNING: {
		        int current_goal = 1;
				double pose_x = waypoints[0].x;
				double pose_y = waypoints[0].y;
		        ...
		        while(ros::ok()) {
					...
					distance = (pose_x - robot_pose.x) * (pose_x - robot_pose.x) + (pose_y - robot_pose.y) * (pose_y - robot_pose.y);
					pose_x = robot_pose.x;
					pose_y = robot_pose.y;
					...
					if(cmd.drive.steering_angle > 0.5){
						...
						dImg1.push_back(dImg);
						steer_folder[index_] = 1;
						steer_folder_total[index_total+index_]=1;
						index_++;
					}
					    ...
					    {
						cv::resize(dImg, dImg, cv::Size(), 0.25, 0.25);
						dImg_temp.push_back(dImg);
						if(index_temp>8)
						{
						    dImg1.push_back(dImg_temp[index_temp-5]);
						    steer_folder[index_]=0;
						    steer_folder_total[index_total+index_]=0;
						    index_++;
						}
					    }
					}
			        ...
			        if (fabs(check_x) < 1.0 && fabs(check_y) < 1.0) {
			            ...
			            pid_ctrl.reset();
						index_total += index_;
						for (int i=0;i<dImg1.size();i++)
						{
						    dImg_total.push_back(dImg1[i]);
						}
						index_=0;
						dImg1.clear();
			            current_goal++;
		...
		dImg_temp.clear();
				...
				if(!collision)
				{
					...
					while(ros::ok())
					{
						...
						img_name << oss1.str() << "image/" << inum << "_" << iter << "_" <<  steer_folder_total[iter]  << ".jpg";
						if(dImg_total[iter].data) {
						    cv::imwrite(img_name.str(), dImg_total[iter]);
						}
/*						if(steer_folder_total[iter]==0)
						{
							img_name << oss1.str() << "0/" << "_" << inum << "_" <<  iter << ".jpg";
							cv::imwrite(img_name.str(), dImg_total[iter]);
						}
						else if(steer_folder_total[iter]==1)
						{
							img_name << oss1.str() << "1/" << "_" << inum << "_" <<  iter << ".jpg";
							cv::imwrite(img_name.str(), dImg_total[iter]);
						}
						else if(steer_folder_total[iter]==-1)
						{
...
//      printf("RRT\n");
        ...
        for(int i=0; i<waypoints.size()-1; i++){
			...
			else if((vec.begin()->x-waypoints[i+1].x)*(vec.begin()->x-waypoints[i+1].x)+(vec.begin()->y-waypoints[i+1].y)*(vec.begin()->y-waypoints[i+1].y) > 1.0 ){
   			    printf("!!!REFIND THE WAY\n");
				cnt++;
			    printf(" x y %.2f %.2f waypoints %.2f %.2f\n", vec.begin()->x, vec.begin()->y,waypoints[i+1].x, waypoints[i+1].y);
			    if(i>0){ 
...
/*	waypoint_candid[0].x = 1;
    ...
    waypoint_candid[1].y = 1;
	waypoint_candid[2].x = -3;
    waypoint_candid[2].y = 5;
	waypoint_candid[3].x = -5.5;
    waypoint_candid[3].y = -3;
	waypoint_candid[4].x = 5;
    waypoint_candid[4].y = -7;
	waypoint_candid[5].x = 7;
    waypoint_candid[5].y = 1;
	waypoint_candid[0].x = -1;
    waypoint_candid[0].y = 0;
	waypoint_candid[0].th = 3*PI/2;
	waypoint_candid[1].x = 1;
    waypoint_candid[1].y = -3;
	waypoint_candid[2].x = 4;
    waypoint_candid[2].y = -1;

*/
    int order[] = {0,1,2,3,4,5,0};
    //int order[] = {0,1,2,3,0};
    int order_size = 2;
